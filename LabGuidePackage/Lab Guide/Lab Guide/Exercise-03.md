...
6. In the **first** condition group, add a condition that the content is labelled with the **Zava Highly Confidential** sensitivity label you created in Challenge 2, using the sensitivity label itself rather than an equivalent sensitive information type.
7. Still inside that **same first group**, add the condition that content is shared from Microsoft 365 **with people outside my organization**. Conditions within a group are joined with **and**, so this group matches labelled content that is genuinely being shared outside the organization.
8. Add a **second** condition group, joined to the first with **or**, that matches content containing **Credit Card Number** or **U.S. Social Security Number (SSN)** with an instance count of 1 or more. Because the groups are joined with **or**, this second group catches unlabelled sensitive content whether or not it has been shared externally. Then set the action to **Restrict access or encrypt the content in Microsoft 365 locations** with **Block only people outside your organization**.
...
> **Note:** The Zava Highly Confidential label is created in Challenge 2 Task 1 and published in Task 2. If it is not yet offered in the sensitivity label picker, wait a few minutes and reopen the condition, since label propagation to the DLP rule options is not instant. No content in this tenant carries the label yet, and nothing you placed in Challenge 1 was shared externally, so the first condition group will report no matches. The second condition group in step 8 is what gives this policy something it can match, which is exactly why the two groups are joined with **or** rather than **and**.
